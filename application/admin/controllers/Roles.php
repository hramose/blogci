...
class Roles extends MY_Controller {
	...
	function save(){
		...
		if($this->form_validation->run() === FALSE){
			...
			if($id === 0){
				...
				json_response(array('success' => TRUE, 'msg' => 'Rol añadido correctamente.'));
			}else{
				check_permission('admin-edit-role');
				$this->db->trans_begin();
				$data = array(
				...
				$this->db->trans_complete();
				cxp_update_cache();
				json_response(array('success' => TRUE, 'msg' => 'Rol editado correctamente.'));
			}
	...
	function delete(){
		...
		$this->db->trans_complete();
		cxp_update_cache();
		json_response(array('success' => TRUE, 'msg' => 'Rol eliminado correctamente.'));
	}
