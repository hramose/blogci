...
class Permissions extends MY_Controller {
  ...
  function loop_parent($perm_parr, $parent_id, $curloop, $curid, $html){
	...
	function save(){
		...
		if($this->form_validation->run() === FALSE){
			...
			if($id === 0){
				...
				json_response(array('success' => TRUE, 'msg' => 'Permisos añadidos correctamente'));
			}else{
				check_permission('admin-edit-permission');
				$this->db->trans_begin();
				$data = array(
				...
				json_response(array('success' => TRUE, 'msg' => 'Permisos editados correctamente.'));
			}
	...
	function delete(){
		...
		cxp_update_cache();
		json_response(array('success' => TRUE, 'msg' => 'Permisos eliminados correctamente.'));
	}
