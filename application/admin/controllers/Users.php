...
class Users extends MY_Controller {
	...
	function save(){
		...
		if($this->form_validation->run() === FALSE){
			...
			if($id === 0){
				...
				json_response(array('success' => TRUE, 'msg' => 'Usuario añadido correcamente.'));
			}else{
				check_permission('admin-edit-user');
				$this->db->trans_begin();
				$data = array(
				...
				$this->db->trans_complete();
				json_response(array('success' => TRUE, 'msg' => 'Usuario editado correctamente.'));
			}
	...
	function delete(){
		...
		$this->db->trans_complete();
		json_response(array('success' => TRUE, 'msg' => 'Usuario eliminado correctamente.'));
	}
	...
	function edit_password(){
		if(strtoupper($_SERVER['REQUEST_METHOD']) === 'POST'){
			...
			if($this->form_validation->run() === FALSE){
				...
				$this->db->update('users', $data);
				json_response(array('success' => TRUE, 'msg' => 'Contraseña cambiada correctamente.'));
			}
		}else{
	...
	function set_perms(){
		if(strtoupper($_SERVER['REQUEST_METHOD']) === 'POST'){
			...
			cxp_update_cache();
			json_response(array('success' => TRUE, 'msg' => 'Datos guardados correctamente'));
		}else{
			$user_id = intval($this->input->get('user_id'));
			$this->db->where('id', $user_id);
			$info = $this->db->get('users')->row();
